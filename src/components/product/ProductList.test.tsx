...
jest.mock('./ProductCard', () => {
  return function MockProductCard({ product, onAddToCart }: { product: Product; onAddToCart: (product: Product) => void }) {
    return (
...
const mockUseProductStore = require('@/hooks/useProductStore').useProductStore as jest.MockedFunction<any>;
const mockUseCartStore = require('@/hooks/useCartStore').useCartStore as jest.MockedFunction<any>;
...
describe('ProductList', () => {
  ...
  describe('Loading state', () => {
    ...
    it('should render multiple skeleton components in correct grid layout when loading', () => {
      ...
      expect(gridContainer).toHaveClass('grid-cols-1', 'sm:grid-cols-2', 'lg:grid-cols-3', 'xl:grid-cols-4', 'gap-6');
    });
  });
  ...
  describe('Edge cases', () => {
    it('should handle single product correctly', () => {
      ...
      expect(screen.getByTestId(`product-card-${singleProduct[0].id}`)).toBeInTheDocument();
    ...
    it('should handle products with edge case data', () => {
      const edgeCaseProducts: Product[] = [
        {
          id: 999,
          title: 'Product with very long title that might wrap to multiple lines',
          price: 0.01,
      ...
      expect(screen.getByTestId(`product-card-${edgeCaseProducts[0].id}`)).toBeInTheDocument();
